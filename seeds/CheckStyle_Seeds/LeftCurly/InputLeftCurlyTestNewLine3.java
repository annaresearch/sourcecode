...
class InputLeftCurlyTestNewLine3 { // ok
  /**
   * @see test method *
   */
  int foo() throws InterruptedException { // ok
    int x = 1;
    int a = 2;
    while (true) { // ok
      try { // ok
  ...
  void method2() { // ok
    ...
    if (flag) {
      String.valueOf("ok");
    } // violation ''{' at column 19 should be on a new line'
  }
}
...
class Absent_CustomFieldSerializer4TestNewLine3 { // ok
...
class ClassWithStaticInitializersTestNewLine3 { // ok
