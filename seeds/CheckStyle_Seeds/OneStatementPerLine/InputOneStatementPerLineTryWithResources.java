...
public class InputOneStatementPerLineTryWithResources {
  ...
  void method() throws IOException {
    ...
    try (s1;
        s2;
        OutputStream s5 = new PipedOutputStream()) {}
    try (s1;
        OutputStream s6 = new PipedOutputStream();
        s2) {}
    try (OutputStream s7 = new PipedOutputStream();
        OutputStream s8 = new PipedOutputStream(); // violation
        s2; ) {}
    try (OutputStream s9 = new PipedOutputStream();
        s2;
        OutputStream s10 = new PipedOutputStream()) // violation
    {}
    try (s1;
    ...
    try (OutputStream s12 = new PipedOutputStream();
        s1;
        OutputStream s3 = new PipedOutputStream() // violation
        ;
        s2; ) {}
    try (OutputStream s12 = new PipedOutputStream();
        s1;
        OutputStream s3 // violation
