...
public class InputOneStatementPerLineMultiline {
  ...
  /** Two multiline assignment (declaration) statements on the same line are illegal. */
  int o = 1, p = 2, r = 5;

  int t; // violation

  /** Two assignment (declaration) statement which are not on the same line are legal. */
  int one = 1, three = 5;
  ...
  int var2 = 6, var3 = 5; // violation

  /** Two statements on the same line (the second is distributed over two lines) are illegal. */
  int var6 = 5;

  int var7 = 6, var8 = 5; // violation
  ...
  /**
   * While theoretically being distributed over three lines, this is a sample of 2 statements on one
   * line.
   */
  int var9 = 1, var10 = 5;
  ...
  /** Two statements on the same lne inside multiline for loop block are illegal. */
  private void foo5() {
    for (int n = 0, k = 1; n < 5; n++, k--) {
      var1++;
      var2++;
  ...
  private void issue2211fail() {
    try (AutoCloseable i = new java.io.PipedReader();
        AutoCloseable k = new java.io.PipedReader(); // violation
        ) {
    } catch (Exception e1) {
    }
  }
  ...
  private void issue2211fail2() {
    try (AutoCloseable i = new StringReader("");
        AutoCloseable k = new StringReader(""); ) { // violation
    } catch (Exception e1) {
    }
  }
}
