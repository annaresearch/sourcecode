...
class InputOperatorWrap2 {
  void test() {
    int x =
        1 + 2 - 3 - // violation ''-' should be on the previous line.'
            4;
    x = x + 2;
    boolean y =
        true && // violation ''&&' should be on the previous line.'
            false;
    y = true && false;
    y = false && true; // violation ''&&' should be on the previous line.'
  ...
  void testAssignment() {
    int x = 0; // violation when checking assignment operators with EOL wrap option
    int y = 0;
  }
...
class badCase22<T extends Foo2 & Bar2> {}
