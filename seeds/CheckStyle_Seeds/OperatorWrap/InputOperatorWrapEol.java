...
class InputOperatorWrapEol {
  ...
  public InputOperatorWrapEol() throws IOException {
    ...
    try (Reader r = // ok
        null) {}
    try (Reader r = null) { // violation ''=' should be on the previous line.'
    }
    int x =
        (1 < 2)
            ? // ok
            false
                ...
                : false // violation '':' should be on the previous line.'
                    ? 2
                    : 3
            : 4; // violation ''?' should be on the previous line.'

    for (int value : // ok
        new int[0]) {}
    for (int value : new int[0]) {} // violation '':' should be on the previous line.'

    int[] a1 = // ok
        {};
    int[] a2 = {}; // violation ''=' should be on the previous line.'
    int[] a3 = { // ok
    };
  }
