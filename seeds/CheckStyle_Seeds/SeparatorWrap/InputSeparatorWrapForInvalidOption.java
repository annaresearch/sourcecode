...
public class InputSeparatorWrapForInvalidOption<
    T extends
        FooForInvalidOption // ok
            & BarForInvalidOption> {
  public void goodCase() throws FooException4IO, BarException4IO {
    int i = 0;
    String s = "ffffooooString";
    s.isEmpty(); // good wrapping
    s.isEmpty();
    try {
      foo(i, s);
    } catch (FooException4IO | BarException4IO e) {
    }
    foo(i, s); // good wrapping
  }
...
class badCaseForInvalidOption<T extends FooForInvalidOption & BarForInvalidOption> {
  ...
  public void goodCaseForInvalidOption(int... aFoo) throws FooException4IO, BarException4IO {
    String s = "ffffooooString";
    s.isEmpty(); // bad wrapping
    try {
      foo(1, s);
    } catch (FooException4IO | BarException4IO e) {
    }

    foo(1, s); // bad wrapping
    int[] i;
  }
