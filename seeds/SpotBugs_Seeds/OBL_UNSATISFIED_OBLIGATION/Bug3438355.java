    ...
    try {
      ...
      statement =
          con.prepareStatement(
              "select count(*) as rowcount from edec_deklarationen_mgt where dko_spediteur_dekl_nr"
                  + " like ? and dko_spediteur_nr = ? ");
      ...
      statement =
          con.prepareStatement(
              "DELETE FROM edec_deklarationen_mgt where dko_spediteur_dekl_nr like ? and"
                  + " dko_spediteur_nr = ?");
      statement.setString(1, spediDeklNr);
      statement.setString(2, spediNr);
      ...
      statement =
          con.prepareStatement(
              "DELETE FROM edec_deklarationen WHERE dek_id IN (SELECT d.dek_id FROM"
                  + " edec_deklarationen d INNER JOIN edec_dekl_kopf k ON d.dek_id = k.dko_dek_id"
                  + " WHERE k.dko_spediteur_dekl_nr like ? )");
      statement.setString(1, spediDeklNr);
      statement.executeQuery();
