...
/**
 * Holder object for the id in the {@link TestLink} annotation. By defining only a private
 * constructor in this abstract class and having both extending classes as inner classes, we assure
 * nobody is able to create other extending classes.
 *
 * @param <T> of the id, either {@link Long} for internal or {@link String} for external IDs.
...
abstract class TestLinkId<T> {
  ...
  /**
   * Private to make sure we only have {@link ExternalTestLinkId} and {@link InternalTestLinkId} as
   * subclasses.
   *
...
public class TestLinkUriResolver {
   ...
   *
   * @param baseUri without <tt>index.php</tt>, eg <tt>http://testlink.sourceforge.net/demo/</tt>.
   */
   ...
   *
   * @param internalTestLinkId id of the testcase
  ...
  private URI fromTestLinkId(final TestLinkId.InternalTestLinkId internalTestLinkId) {
    return baseUri.resolve(
        String.format("lib/testcases/tcPrint.php?testcase_id=%s", internalTestLinkId.getId()));
  }

  /**
   * http://testlink.sourceforge.net/demo/lib/testcases/archiveData.php?targetTestCase=SM-1&edit=testcase&allowedit=0.
   *
   * @param externalTestLinkId id of the testcase
   * @return an URI pointing to the printview of the last version of the testcase description.
   */
  private URI fromTestLinkId(final TestLinkId.ExternalTestLinkId externalTestLinkId) {
    return baseUri.resolve(
        String.format(
            "lib/testcases/archiveData.php?targetTestCase=%s&edit=testcase&allowedit=0",
   ...
   *
   * <p>http://testlink.sourceforge.net/demo/lib/testcases/tcPrint.php?testcase_id=2750
   * http://testlink.sourceforge.net/demo/lib/testcases/archiveData.php?targetTestCase=SM-1&edit=testcase&allowedit=0
   *
   * @param testLinkId of the test case.
