...
class ReflectionOnNonRuntimeAnnotationCheck {
  ...
  void foo(Method m, Class<?> c) {
    m.isAnnotationPresent(
        Override
            .class); // Noncompliant [[sc=27;ec=41]] {{"@Override" is not available at runtime and
    // cannot be seen with reflection.}}
    c.isAnnotationPresent(
        Override
            .class); // Noncompliant {{"@Override" is not available at runtime and cannot be seen
    // with reflection.}}
    ...
    m.isAnnotationPresent(
        this.annotation); // Compliant, we can't know what the field annotation is returning.
  }
  ...
  private void addMethod(Class<?> c) {
    ...
    boolean annotation2 =
        c.isAnnotationPresent(
            Expose2
                .class); // Noncompliant {{"@Expose2" is not available at runtime and cannot be seen
    // with reflection.}}
    boolean annotation3 =
        c.isAnnotationPresent(
            Expose3
                .class); // Noncompliant {{"@Expose3" is not available at runtime and cannot be seen
    // with reflection.}}
    boolean annotation4 =
        c.isAnnotationPresent(
            Expose4
                .class); // Noncompliant {{"@Expose4" is not available at runtime and cannot be seen
    // with reflection.}}
  }
}
