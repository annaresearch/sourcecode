...
class LambdaA {
  void fun() {
    IntStream.range(1, 5)
        ...
        .map(
            x ->
                square(
                    x)) // Noncompliant [[sc=16;ec=18]] {{Replace this lambda with method reference
        // 'this::square'.}}
        .map(
            x -> { // Noncompliant
              return square(x);
  ...
  Collection<Number> values =
      transform(
          input ->
              getValueProvider()
                  .apply(input)
                  .getValue() // cannot be replaced by  a method reference.
          );

  Collection<Number> values2 =
      transform2(
          (input, input2) ->
              getValueProvider().apply(input)); // cannot be replaced by  a method reference.
  ...
  Collection transform(F f) {
    return null;
  }

  Collection transform2(F2 f) {
    return null;
  }
  ...
  void nullChecks(List<String> strings, String s2) {
    strings.stream()
        .filter(s -> s != null); // Noncompliant {{Replace this lambda with method reference
    // 'Objects::nonNull'.}}
    strings.stream()
        .filter(
            s -> {
              return s != null;
            }); // Noncompliant {{Replace this lambda with method reference 'Objects::nonNull'.}}
    strings.stream()
        .filter(s -> (s) == null); // Noncompliant {{Replace this lambda with method reference
    // 'Objects::isNull'.}}
    strings.stream()
        .filter(s -> null == s); // Noncompliant {{Replace this lambda with method reference
    // 'Objects::isNull'.}}
    strings.stream()
        .filter(s -> (((s == null)))); // Noncompliant {{Replace this lambda with method reference
    // 'Objects::isNull'.}}
...
class CastCheck {
  ...
  void testInstanceOf(Object param) {
    barbar((o) -> o instanceof String); // Noncompliant {{Replace this lambda with method reference
    // 'String.class::isInstance'.}}
    barbar(String.class::isInstance); // Compliant

    barbar4((o) -> o instanceof TestA); // Noncompliant {{Replace this lambda with method reference
    // 'TestA.class::isInstance'.}}
    barbar4(TestA.class::isInstance); // Compliant

    barbar3(List.class::isInstance); // Compliant
    barbar3((o) -> o instanceof List); // Noncompliant {{Replace this lambda with method reference
    // 'List.class::isInstance'.}}

    barbar5(TestA[].class::isInstance); // Compliant
    barbar5(
        (o) -> o instanceof TestA[]); // Noncompliant {{Replace this lambda with method reference
    // 'TestA[].class::isInstance'.}}
    ...
    barbar7((o) -> o instanceof List); // Noncompliant {{Replace this lambda with method reference
    // 'List.class::isInstance'.}}

    barbar8(List[].class::isInstance); // Compliant
    barbar8((o) -> o instanceof List[]); // Noncompliant {{Replace this lambda with method reference
    // 'List[].class::isInstance'.}}

    barbar9(char.class::isInstance); // Compliant
    barbar9(Character.class::isInstance); // Compliant
    barbar9(
        (o) -> o instanceof Character); // Noncompliant {{Replace this lambda with method reference
    // 'Character.class::isInstance'.}}
  }

  void testInstanceOf2(Object param) {
    barbar(
        (o) -> { // Noncompliant {{Replace this lambda with method reference
          // 'String.class::isInstance'.}}
  ...
  void testCasts(Object param) {
    bar((o) -> (String) o); // Noncompliant {{Replace this lambda with method reference
    // 'String.class::cast'.}}
    bar(String.class::cast); // Compliant

    bar4((o) -> (TestA) o); // Noncompliant {{Replace this lambda with method reference
    // 'TestA.class::cast'.}}
    ...
    bar5((o) -> (TestA[]) o); // Noncompliant {{Replace this lambda with method reference
    // 'TestA[].class::cast'.}}

    bar6(TestA[][].class::cast); // Compliant
    bar6((o) -> (TestA[][]) o); // Noncompliant {{Replace this lambda with method reference
    // 'TestA[][].class::cast'.}}
    ...
    bar9((o) -> (char) o); // Noncompliant {{Replace this lambda with method reference
    // 'char.class::cast'.}}
    bar9((o) -> (Character) o); // Noncompliant {{Replace this lambda with method reference
    // 'Character.class::cast'.}}

    "abc"
        .chars()
        .mapToObj(
            i -> (char) i); // Compliant, char::cast takes an Object as its argument causing i to be
    // implicitly converted to Integer,
    "abc"
        .chars()
        .mapToObj(
            char.class
                ::cast); // since casting Integer to char is invalid, this change would turn a valid
    // cast into an invalid one
  }

  void testCasts2(Object param) {
    bar(
        (o) -> { // Noncompliant {{Replace this lambda with method reference 'String.class::cast'.}}
          return (String) o;
        });
  }
  ...
  void quickFixes(List<String> strings) {
    IntStream.range(1, 5)
        .forEach(
            x ->
                staticMethod(
                    x)); // Noncompliant [[sc=37;ec=39;quickfixes=qf1]] {{Replace this lambda with
    // method reference 'CastCheck::staticMethod'.}}
    ...
    IntStream.range(1, 5)
        .forEach(
            x ->
                notStatic(
                    x)); // Noncompliant [[sc=37;ec=39;quickfixes=qf2]] {{Replace this lambda with
    // method reference 'this::notStatic'.}}
    ...
    IntStream.range(1, 5)
        .forEach(
            x ->
                n.takeInt(
                    x)); // Noncompliant [[sc=37;ec=39;quickfixes=qf3]] {{Replace this lambda with
    // method reference 'n::takeInt'.}}
    // fix@qf3 {{Replace with "n::takeInt"}}
    // edit@qf3 [[sc=35;ec=52]] {{n::takeInt}}
    IntStream.range(1, 5)
        .forEach(
            x ->
                NestedStatic.takeIntStatic(
                    x)); // FN, do not report an issue because NestedStatic is not final

    IntStream.range(1, 5)
        .forEach(
            x ->
                new Nested(
                    x)); // Noncompliant [[sc=37;ec=39;quickfixes=qf_new_class]] {{Replace this
    // lambda with method reference 'Nested::new'.}}
    // fix@qf_new_class {{Replace with "Nested::new"}}
    // edit@qf_new_class [[sc=35;ec=53]] {{Nested::new}}
    IntStream.range(1, 5)
        .forEach(
            x ->
                new CastCheck.Nested(
                    x)); // Noncompliant [[sc=37;ec=39;quickfixes=qf_new_class2]] {{Replace this
    // lambda with method reference 'CastCheck.Nested::new'.}}
    ...
    IntStream.range(1, 5)
        .forEach(
            x ->
                new NestedStatic.NestedInNested(
                    x)); // Noncompliant [[sc=37;ec=39;quickfixes=qf_new_class3]] {{Replace this
    // lambda with method reference 'NestedStatic.NestedInNested::new'.}}
    ...
    strings.stream()
        .map(s -> s.toLowerCase()) // Noncompliant [[sc=14;ec=16;quickfixes=qf4]] {{Replace this
        // lambda with method reference 'String::toLowerCase'.}}
        // fix@qf4 {{Replace with "String::toLowerCase"}}
        // edit@qf4 [[sc=12;ec=32]] {{String::toLowerCase}}
        .map(
            s -> { // Noncompliant [[sc=14;ec=16;quickfixes=qf5]]
              return s.toLowerCase();
            })
        ...
        .forEach(
            x ->
                System.out.println(
                    x)); // Noncompliant [[sc=18;ec=20;quickfixes=qf6]] {{Replace this lambda with
  ...
  class Nested {
    {
      IntStream.range(1, 5)
          .forEach(
              x ->
                  takeInt(
                      x)); // Noncompliant [[sc=39;ec=41;quickfixes=qf_init]] {{Replace this lambda
      // with method reference 'Nested.this::takeInt'.}}
      // Could be simpler: "this::takeInt", but add the class name since we can not get the
      // enclosing method
      ...
      IntStream.range(1, 5)
          .forEach(
              x ->
                  notStatic(
                      x)); // Noncompliant [[sc=39;ec=41;quickfixes=qf_init2]] {{Replace this lambda
      // with method reference 'CastCheck.this::notStatic'.}}
    ...
    void quickFixInNestedClass() {
      IntStream.range(1, 5)
          .forEach(
              x ->
                  notStatic(
                      x)); // Noncompliant [[sc=39;ec=41;quickfixes=qf_this]] {{Replace this lambda
      // with method reference 'CastCheck.this::notStatic'.}}
      ...
      IntStream.range(1, 5)
          .forEach(
              x ->
                  staticMethod(
                      x)); // Noncompliant [[sc=39;ec=41;quickfixes=qf_this2]] {{Replace this lambda
      // with method reference 'CastCheck::staticMethod'.}}
      ...
      void quickFixInNestedClass() {
        IntStream.range(1, 5)
            .forEach(
                x ->
                    notStatic(
                        x)); // Noncompliant [[sc=41;ec=43;quickfixes=qf_this3]] {{Replace this
        // lambda with method reference 'CastCheck.this::notStatic'.}}
  ...
  void quickFixInNestedClass() {
    Stream.of(new NestedExtend())
        .forEach(
            x -> x.doSomething()); // Noncompliant [[sc=45;ec=47;quickfixes=qf_method_override]]
    // fix@qf_method_override {{Replace with "Nested::doSomething"}}
    // edit@qf_method_override [[sc=43;ec=63]] {{Nested::doSomething}}

    Stream.of(new NestedExtendOverrideTakeInt())
        .forEach(
            x -> x.doSomething()); // Noncompliant [[sc=60;ec=62;quickfixes=qf_method_override2]]
