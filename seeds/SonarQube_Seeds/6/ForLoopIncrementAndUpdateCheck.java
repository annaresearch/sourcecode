...
class ForLoopIncrementAndUpdateCheck {
  ...
  Object foo() {
    ...
    for (i = 0;
        i < 10;
        j++, m[0]++) { // Noncompliant [[sc=5;ec=8;secondary=13]] {{Move the update of "i" into
      // this loop's update clause.}}
      i++;
    }
    ...
    for (i = 0;
        k < 10 && l < 10;
        i++,
            j++) { // Noncompliant [[secondary=30]] {{Move the update of "k" into this loop's update
      // clause.}}
      l -= 42;
      k++;
    }
    for (i = 0;
        k < 10 && l < 10;
        i++,
            j++) { // Noncompliant [[secondary=33,34]] {{Move the update of "k","l" into this loop's
      // update clause.}}
      l--;
      k++;
    }
    ...
    for (i = 0;
        a.myField < 10;
        j++) { // Noncompliant {{Move the update of "myField" into this loop's update clause.}}
      (a.myField)++;
    }
    ...
    for (ExecutionState current = state;
        !current.equals(this);
        current = current.parentState) {} // Compliant
  ...
  private boolean someCondition(int i) {
    return true;
  }
