...
class FloatEquality {
  void foo() {
    ...
    int i = 1;
    if (f1
        == f2) {} // Noncompliant [[sc=11;ec=13]] {{Equality tests should not be made with floating
    // point values.}}
    if (f1
        != f2) {} // Noncompliant {{Equality tests should not be made with floating point values.}}
    if (f1
        == i) {} // Noncompliant {{Equality tests should not be made with floating point values.}}
