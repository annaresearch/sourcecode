...
class ExpressionComplexityCheckCustom {
  ...
  boolean d =
      true && false && true && false && true
          && true; // Noncompliant [[effortToFix=1]] {{Reduce the number of conditional operators
  // (5) used in the expression (maximum allowed 4).}}
  ...
  void f() {
    if ((true ? 0 : 1) == 0
        || false
        || true && false && true
        || false) { // Noncompliant [[effortToFix=2]] {{Reduce the number of conditional operators
      // (6) used in the expression (maximum allowed 4).}}
    }
  }
