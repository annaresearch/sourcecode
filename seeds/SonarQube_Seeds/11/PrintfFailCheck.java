...
public class PrintfFailCheck {
  void foo(Calendar c) throws java.io.IOException {
    ...
    String.format(
        "Duke's Birthday year is %tX",
        12l); // Noncompliant {{X is not a supported time conversion character}}
    ...
    String.format(
        "%1$d %2$d %9$-3.3s",
        1, 2, "hello"); // Noncompliant {{Not enough arguments to feed formater at index 9: '%9$'.}}
    String.format(
        "%12$s",
        1, 2,
        "hello"); // Noncompliant {{Not enough arguments to feed formater at index 12: '%12$'.}}
    ...
    String.format(
        "%< is equals to %d",
        2); // Noncompliant {{The argument index '<' refers to the previous format specifier but
    // there isn't one.}}
    ...
    String.format(
        "%d %d",
        new Object[] {1, 2, 3}); // Compliant, too many arguments but no errors, reported by S3457
    ...
    String.format(
        ...
        IntStream.range(0, 1)
            ...
            .toArray()); // FN, acceptable to avoid noise
    String.format(
        "%d%d",
        IntStream.range(0, 2).mapToObj(Integer::valueOf).toArray(Object[]::new)); // Compliant
    String.format(
        "Result %s %s",
        new Exception(), new Exception(), new Exception()); // Compliant, reported by S3457
    String.format(
        "Result %s %s",
        new Object[] {
          new Exception(), new Exception(), new Exception()
        }); // Compliant, reported by S3457
    ...
    String.format(
        "Result %s %s", new Object[] {new Exception()}); // Noncompliant {{Not enough arguments.}}
    ...
    String.format(
        "Duke's Birthday year is %t",
        12l); // Noncompliant {{Time conversion requires a second character.}}
    ...
    String.format(
        "Duke's Birthday year is %tH",
        loc); // Noncompliant {{Time argument is expected (long, Long, Calendar, Date and
    // TemporalAccessor).}}
    ...
    // ================================================================================================
    "The value of my integer is %d"
        .formatted("Hello World"); // Noncompliant {{An 'int' is expected rather than a String.}}
    "First {0} and then {1}".formatted("foo", "bar");
    "Duke's Birthday year is %tX"
        .formatted(12l); // Noncompliant {{X is not a supported time conversion character}}
    ...
    "%1$d %2$d %9$-3.3s"
        .formatted(
            1, 2,
            "hello"); // Noncompliant {{Not enough arguments to feed formater at index 9: '%9$'.}}
    "%12$s"
        .formatted(
            1, 2,
            "hello"); // Noncompliant {{Not enough arguments to feed formater at index 12: '%12$'.}}
    ...
    pr.format("value is " + "asd"); // Compliant
    ...
    MessageFormat.format(
        "Result ' {0}",
        14); // Compliant, wrong string formatting but no error: will be reported by S3457
    MessageFormat.format(
        "Result {{{0}}.", 14); // Noncompliant {{Single left curly braces "{" must be escaped.}}
    ...
    MessageFormat.format(
        ...
        new Object[] {
          0.07
        }); // Compliant, wrong string formatting but no error: will be reported by S3457
    MessageFormat.format(
        "{0,number,#.#}{1}",
    ...
    logger.log(
        ...
        "Result {0}"); // Compliant, wrong string formatting but no error: will be reported by S3457
    logger.log(
        java.util.logging.Level.SEVERE,
        "Result {1}"); // Compliant, wrong string formatting but no error: will be reported by S3457
    ...
    logger.log(
        java.util.logging.Level.SEVERE,
        "Result {0}",
        new Exception()); // Compliant, wrong string formatting but no error: will be reported by
    // S3457
    logger.log(
        java.util.logging.Level.SEVERE,
        "message {0}",
    ...
    logger.log(
        java.util.logging.Level.SEVERE,
        "Result {0} and {1}!",
        new Integer[] {14, 18, 12}); // compliant
    logger.log(
    ...
    logger.log(
        java.util.logging.Level.SEVERE,
        "message {0} " + value,
        value); // Compliant, reported by S3457
    logger.log(
    ...
    slf4jLog.debug(
        marker,
        "message {}"); // Compliant, wrong string formatting but no error: will be reported by S3457
    ...
    slf4jLog.error(
        "message {} {}",
        1); // Compliant, wrong string formatting but no error: will be reported by S3457
    slf4jLog.error(
        "message {} {}",
        1,
        new Exception()); // Compliant, wrong string formatting but no error: will be reported by
    ...
    slf4jLog.warn("message {}", 1);
    slf4jLog.warn(
        "Output on the error channel detected: this is probably due to a problem on pylint's"
            + " side.");
    slf4jLog.warn(
        "message {}",
        new Exception()); // Compliant, wrong string formatting but no error: will be reported by
    ...
    String fileKey = "key";
    slf4jLog.warn(
        "The resource for '{}' is not found, drilling down to the details of this test won't be"
            + " possible",
        fileKey);
    slf4jLog.warn(
        "The resource for is not found, drilling down to the details of this test won't be"
    ...
    log4j.error(
        "message {} {}", 1, 2, 3); // Compliant, detected by S3457 "3rd argument is not used."
    log4j.error(
        "message {} {}",
        1,
        2,
        new Exception().toString()); // Compliant, detected by S3457 "3rd argument is not used."
    log4j.error(
        "message ",
        () -> 1); // Compliant, detected by S3457 "String contains no format specifiers."
    ...
    log4j.warn(
        "message {} {} {}",
        1,
        2); // Compliant, wrong string formatting but no error: will be reported by S3457
    ...
    log4j.debug(
        "message %s message %d",
        "hello",
        "world"); // Compliant, wrong string formatting but no error: will be reported by S3457
    log4j.debug(
        "message %s message %d %s",
        "hello", 42); // Compliant, wrong string formatting but no error: will be reported by S3457
