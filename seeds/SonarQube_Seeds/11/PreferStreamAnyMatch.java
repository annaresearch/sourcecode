...
class PreferStreamAnyMatch {
  ...
  void test(Random r) {
    IntStream.range(0, 10)
        .filter(i -> true)
        .findFirst()
        .isPresent(); // Noncompliant [[sc=28;ec=67]] {{Replace this
    // "filter().findFirst().isPresent()" chain with "anyMatch()".}}
    IntStream.range(0, 10)
        .filter(i -> true)
        .findAny()
        .isPresent(); // Noncompliant [[sc=28;ec=65]] {{Replace this
    // "filter().findAny().isPresent()" chain with "anyMatch()".}}
  ...
  void anymatch(Stream<Object> stream) {
    boolean match =
        !stream
            .filter(o -> o instanceof PreferStreamAnyMatch)
            .filter(o -> o != null)
            .anyMatch(o -> true); // Noncompliant {{Replace this negation and "anyMatch()" with
    // "noneMatch()".}}
    match =
        !stream.anyMatch(
            o -> !true); // Noncompliant {{Replace this double negation with "allMatch()" and
    // positive predicate.}}
    match =
        stream
            .map(o -> o.equals(""))
            .anyMatch(
                Boolean::booleanValue); // Noncompliant [[sc=43;ec=51]] {{Use mapper from "map()"
    // directly as predicate in "anyMatch()".}}
