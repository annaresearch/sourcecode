...
public class PrintfMisuseCheck {
  ...
  java.util.logging.Logger loggerField =
      java.util.logging.Logger.getAnonymousLogger("som.foo.resources.i18n.LogMessages");
  ...
  void foo(Calendar c) throws IOException {
    ...
    String.format(
        "First {0} and then {1}",
        "foo", "bar"); // Noncompliant  {{Looks like there is a confusion with the use of
    // java.text.MessageFormat, parameters will be simply ignored here}}
    String.format("{1}", "foo", "bar"); // Noncompliant
    String.format("Duke's Birthday year is %tX", 12l);
    String.format(
        "Display %3$d and then %d", 1, 2, 3); // Noncompliant {{2nd argument is not used.}}
    String.format(
        "Display %2$d and then %2$d", 1, 2); // Noncompliant {{first argument is not used.}}
    String.format(
        "Too many arguments %d and %d", 1, 2, 3); // Noncompliant {{3rd argument is not used.}}
    String.format("Not enough arguments %d and %d", 1);
    String.format(
        "%1$d %2$d %9$-3.3s",
        1, 2, "hello"); // Compliant - not enough arguments but this will be caught by S2275
    String.format(
        "%12$s",
        1, 2, "hello"); // Compliant - not enough arguments but this will be caught by S2275
    String.format(
        "First Line\n %d",
        1); // Noncompliant {{%n should be used in place of \n to produce the platform-specific line
    // separator.}}
    ...
    String.format(
        "Is myObject null ? %b", myObject); // Noncompliant {{Directly inject the boolean value.}}
    String.format("boolean are %b, %b, %b and %b", true, Boolean.TRUE, false, Boolean.FALSE);
    String.format(
        "value is " + value); // Noncompliant {{Format specifiers should be used instead of string
    // concatenation.}}
    String.format((String) "");
    String.format(
        value + "value is "); // Noncompliant {{Format specifiers should be used instead of string
    // concatenation.}}
    String.format(
        "value is "
            + NON_COMPILE_TIME_CONSTANT); // Noncompliant {{Format specifiers should be used instead
    // of string concatenation.}}
    String.format("value is " + COMPILE_TIME_CONSTANT);
    String.format(
        "%s " + value,
        new Exception()); // Noncompliant {{Format specifiers should be used instead of string
    // concatenation.}}
    String.format(
        "string without arguments"); // Noncompliant {{String contains no format specifiers.}}
    ...
    String.format(
        "%d%d",
        IntStream.range(0, 2).mapToObj(Integer::valueOf).toArray(Object[]::new)); // Compliant
    String.format(
        "Result %s %s",
        new Exception(),
        new Exception(),
        new Exception()); // Noncompliant {{3rd argument is not used.}}
    String.format(
        "Result %s %s",
        new Object[] {
          new Exception(), new Exception(), new Exception()
    ...
    String.format(
        "Too many arguments %d and %d and %d",
        1, 2, 3, 4); // Noncompliant {{4th argument is not used.}}
    ...
    System.out.printf(
        "%1$ty_%1$tm_%1$td_%1$tH_%1$tM_%1$tS", java.time.LocalDateTime.now()); // Compliant
    ...
    "First {0} and then {1}"
        .formatted("foo", "bar"); // Noncompliant  {{Looks like there is a confusion with the use of
    // java.text.MessageFormat, parameters will be simply ignored here}}
    ...
    pr.format(
        "string without arguments"); // Noncompliant  {{String contains no format specifiers.}}
    pr.format(
        loc, "string without arguments"); // Noncompliant  {{String contains no format specifiers.}}
    pr.printf(
        "string without arguments"); // Noncompliant  {{String contains no format specifiers.}}
    pr.printf(
        loc, "string without arguments"); // Noncompliant  {{String contains no format specifiers.}}
    ...
    String.format("line1 %s " + "line2", "myValue");

    // MessageFormat
    // ===================================================================================================
    MessageFormat messageFormat = new MessageFormat("{0}");
    messageFormat.format(
        new Object(), new StringBuffer(), new FieldPosition(0)); // Compliant - Not considered
    ...
    MessageFormat.format(
        "Result '{0}'", 14); // Noncompliant {{String contains no format specifiers.}}
    ...
    MessageFormat.format(
        "Result {0}!",
        myObject
            .toString()); // Noncompliant {{No need to call "toString()" method as formatting and
    // string conversion is done by the Formatter.}}
    MessageFormat.format("Result {0}!", myObject.hashCode()); // Compliant
    MessageFormat.format(
        "Result yeah!", 14); // Noncompliant {{String contains no format specifiers.}}
    ...
    MessageFormat.format(
        "Result {0} and {0}!", 14, 42); // Noncompliant {{2nd argument is not used.}}
    MessageFormat.format(
        "Result {0, number, integer} and {1, number, integer}!", 14, 42); // compliant
    MessageFormat.format(
        "Result {0} and {1}!", 14, 42, 128); // Noncompliant {{3rd argument is not used.}}
    MessageFormat.format("{0,number,#.#}{1}", new Object[] {0.07, "$"}); // Compliant
    MessageFormat.format(
        "{0,number,#.#}{1}", new Object[] {0.07}); // Noncompliant {{Not enough arguments.}}
    MessageFormat.format(
        "{0,number,#.#}{1}",
        objs); // Compliant - skipped as the array is not initialized in the method invocation
    ...
    MessageFormat.format(
        "Result {0}", 1, new Exception()); // Noncompliant {{2nd argument is not used.}}
    ...
    MessageFormat.format(
        "Result {0} {1}", 1, 2, new Exception()); // Noncompliant {{3rd argument is not used.}}
    MessageFormat.format(
        "Result {0} {1}", new Exception()); // Noncompliant {{Not enough arguments.}}
    ...
    logger.log(
        ...
        "Result '{0}'",
        14); // Noncompliant {{String contains no format specifiers.}}
    logger.log(
        java.util.logging.Level.SEVERE,
        "Result ' {0}",
        14); // Noncompliant {{Single quote "'" must be escaped.}}
    logger.log(
        java.util.logging.Level.SEVERE,
        "Result {0}!",
        myObject
    ...
    logger.log(
        java.util.logging.Level.SEVERE,
        "Result yeah!",
        new Exception()); // compliant, throwable parameter
    logger.log(
        java.util.logging.Level.SEVERE,
        "message",
        new Object[] {new Exception()}); // Noncompliant {{String contains no format specifiers.}}
    logger.log(
        java.util.logging.Level.SEVERE,
        "message {0}",
        new Object[] {new Exception()}); // Compliant, exceptions are not removed from argument list
    logger.log(
        java.util.logging.Level.SEVERE,
        "message {0}",
        new Object[] {1, new Exception()}); // Noncompliant {{2nd argument is not used.}}
    logger.log(
        java.util.logging.Level.SEVERE,
        "message {0} {1}",
        new Object[] {1, new Exception()}); // Compliant
    logger.log(
        java.util.logging.Level.SEVERE,
        "message {0} {1}",
        new Object[] {1, 2, new Exception()}); // Noncompliant {{3rd argument is not used.}}
    logger.log(
        java.util.logging.Level.SEVERE,
        "message {0} {1}",
        new Object[] {new Exception()}); // Noncompliant {{Not enough arguments.}}
    logger.log(
        java.util.logging.Level.SEVERE,
        "Result {0}",
        new Exception()); // Noncompliant {{Not enough arguments.}}
    logger.log(
        java.util.logging.Level.SEVERE, "Result {0}"); // Noncompliant {{Not enough arguments.}}
    logger.log(
        java.util.logging.Level.SEVERE, "Result {1}"); // Noncompliant {{Not enough arguments.}}
    logger.log(
        java.util.logging.Level.SEVERE,
        "Result {1}",
        new Integer[] {14}); // Noncompliant {{Not enough arguments.}}
    logger.log(
        java.util.logging.Level.SEVERE,
        "Result {0} and {1}!",
        14); // Noncompliant {{Not enough arguments.}}
    logger.log(
        java.util.logging.Level.SEVERE, "Result {0} and {1}!", new Integer[] {14, 18}); // compliant
    logger.log(
        java.util.logging.Level.SEVERE,
        "Result {0} and {1}!",
        new Integer[] {14, 18, 12}); // Noncompliant {{3rd argument is not used.}}
    logger.log(
        java.util.logging.Level.SEVERE, "{0,number,#.#}{1}", new Object[] {0.07, "$"}); // Compliant
    logger.log(
        java.util.logging.Level.SEVERE,
        "{0,number,#.#}{1}",
        new Object[] {0.07}); // Noncompliant {{Not enough arguments.}}
    ...
    logger.log(
        java.util.logging.Level.SEVERE,
        "{0,number,#.#}{1}",
        new Object[42]); // Compliant - Not considered
    logger.log(
        java.util.logging.Level.SEVERE,
        "value=\"'{'{0}'}'{1}\"",
    ...
    logger.log(
        ...
        new Object[] {
          myObject.toString()
        }); // Noncompliant {{No need to call "toString()" method as formatting and string
    // conversion is done by the Formatter.}}
    logger.log(
        java.util.logging.Level.SEVERE,
        "Result {0}!",
        new String[] {
          myObject.toString()
        }); // Noncompliant {{No need to call "toString()" method since an array of Objects can be
    // used here.}}
    logger.log(
        java.util.logging.Level.SEVERE,
        "Result {0} {1}!",
    ...
    logger.log(
        java.util.logging.Level.SEVERE,
        "message "
            + value); // Noncompliant {{Format specifiers or lambda should be used instead of string
    // concatenation.}}
    logger.log(
        java.util.logging.Level.SEVERE,
        "message " + value,
        new Exception()); // Noncompliant {{Lambda should be used to defer string concatenation.}}
    logger.log(
        java.util.logging.Level.SEVERE, new Exception(), () -> "Result " + value); // Compliant
    logger.log(
        java.util.logging.Level.SEVERE,
        "message {0} " + value,
        value); // Noncompliant {{Format specifiers or lambda should be used instead of string
    ...
    java.util.logging.Logger logger2 =
        java.util.logging.Logger.getLogger("som.foo", "som.foo.resources.i18n.LogMessages");
    ...
    logger4.log(
        level, "message {1}", new Object[] {param1}); // Noncompliant {{Not enough arguments.}}
    logger4.log(
        level,
        "message {0}",
        new Object[] {param1, new Exception()}); // Noncompliant {{2nd argument is not used.}}
    logger4.log(level, "message {0} {1}", new Object[] {param1, param2});
    logger4.log(
        level,
        "message {0} {1}",
        new Object[] {param1, param2, param3}); // Noncompliant {{3rd argument is not used.}}
    logger4.log(
        level,
        "message "
            + param1); // Noncompliant {{Format specifiers or lambda should be used instead of
    ...
    // slf4jLog
    // ========================================================================================================
    // slf4jLog is a facade, various logging frameworks can be used under it. It implies that we
    // will only report issues when
    // there are obvious mistakes, not when it depends on the underlying framework (even if it works
    // correctly with the common one).
    ...
    slf4jLog.debug(
        marker, "message {} {} {}", new Object[] {1, 2}); // Noncompliant {{Not enough arguments.}}
    ...
    slf4jLog.error(
        "message {} {}",
        1,
        new Exception()); // Compliant, only a problem when we have only one throwable argument.
    ...
    slf4jLog.error(
        ...
        1,
        2,
        new Exception().toString()); // Noncompliant {{3rd argument is not used.}}
    slf4jLog.error("message {} {}", 1, new Exception()); // Compliant
    slf4jLog.error("message {} {}", 1, 2, new Exception()); // Compliant
    slf4jLog.error(
        "message {} {} {}", 1, new Exception()); // Noncompliant {{Not enough arguments.}}
    slf4jLog.error("message {} {} {}", 1, 2, new Exception()); // Compliant
    slf4jLog.error("message {} {} {}", 1, 2, 3, new Exception()); // Compliant
    slf4jLog.error(
        "message {} {} {}", 1, 2, new Exception(), 3); // Noncompliant {{4th argument is not used.}}
    slf4jLog.error(
        "message {} {} {}",
        1,
        2,
        3,
        4,
        new Exception()); // Noncompliant {{4th argument is not used.}}

    slf4jLog.info(
        "message {}", new Object[] {new Exception()}); // Noncompliant {{Not enough arguments.}}
    slf4jLog.info("message {}", new Object[] {1, new Exception()}); // Compliant
    slf4jLog.info(
        "message {}",
        new Object[] {new Exception(), 1}); // Noncompliant {{2nd argument is not used.}}
    slf4jLog.info("message {} {}", new Object[] {1, new Exception()}); // Compliant
    slf4jLog.info("message {} {}", new Object[] {1, 2, new Exception()}); // Compliant
    slf4jLog.info(
        "message {} {}",
        new Object[] {1, 2, 3, new Exception()}); // Noncompliant {{3rd argument is not used.}}
    ...
    slf4jLog.error(
        "message: " + value); // Noncompliant {{Format specifiers should be used instead of string
    // concatenation.}}
    slf4jLog.error(
        "message: {}" + value,
        value); // Noncompliant {{Format specifiers should be used instead of string
    // concatenation.}}
    slf4jLog.error(
        "message:" + value,
        new Exception()); // Compliant, using error(String, Object, Object) would compile, but no
    // guarantee that
    // the last argument will be treated as a Throwable
    slf4jLog.error(
        "message: {}",
        value,
        new Exception()); // Compliant, but may not print the stack trace depending on the
    ...
    log4j.log(
        ...
        "message",
        1); // Noncompliant {{String contains no format specifiers.}}
    log4j.log(
        org.apache.logging.log4j.Level.DEBUG,
        "message {}"); // Noncompliant {{Not enough arguments.}}
    log4j.log(
        org.apache.logging.log4j.Level.DEBUG,
        "message {}",
        new Exception()); // Noncompliant {{Not enough arguments.}}
    ...
    log4j.log(
        org.apache.logging.log4j.Level.DEBUG,
        "message {} {}",
        1); // Noncompliant {{Not enough arguments.}}
    ...
    formatterLogger.log(
        org.apache.logging.log4j.Level.DEBUG, "message %d", 1, "hello"); // Noncompliant
    ...
    formatterLogger.debug(
        "message %s message", "hello", 42); // Noncompliant {{2nd argument is not used.}}
    formatterLogger.debug(
        "message %s {} message", "hello", 42); // Noncompliant {{2nd argument is not used.}}
    formatterLogger.debug(
        "message %s {} %s message",
        "hello",
        42); // Noncompliant - false-positive, the rule doesn't know it's a "FormatterLogger"
    formatterLogger.printf(
        org.apache.logging.log4j.Level.DEBUG, "message %s {} %s message", "hello", 42);
    formatterLogger.debug(
        "message %s {} {} message",
        "hello", 42); // false-negative, the rule doesn't know it's a "FormatterLogger"
    formatterLogger.printf(
        org.apache.logging.log4j.Level.DEBUG,
        "message %s {} {} message",
        "hello",
        42); // Noncompliant {{2nd argument is not used.}}

    log4j.printf(
        org.apache.logging.log4j.Level.DEBUG,
        "message"); // Noncompliant {{String contains no format specifiers.}}
    log4j.printf(
        org.apache.logging.log4j.Level.DEBUG,
        "message %s %d",
        "hello",
        42); // Compliant - Java formatters
    log4j.printf(
        org.apache.logging.log4j.Level.DEBUG,
        "message %s",
        "hello",
    ...
    log4j.error(
        "message {} {}",
        1,
        new Exception()); // Compliant, only a problem when we have one throwable argument.
    ...
    log4j.debug(
        "message %s message %d",
        "hello", "world"); // Noncompliant {{An 'int' is expected rather than a String.}}
    ...
    // concatenation.}}
    log4j.error(
        "message: {}", value, new Exception()); // Compliant, print the stack-trace as expected
  }
