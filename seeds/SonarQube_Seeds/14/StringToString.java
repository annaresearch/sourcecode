...
public class StringToString {
  ...
  public void method() {
    ...
    array[0].toString(); // Noncompliant [[sc=5;ec=10]] {{"array" is an array of strings, there's no
    // need to call "toString()".}}
    string.toString(); // Noncompliant {{"string" is already a string, there's no need to call
    // "toString()" on it.}}
    string
        .toUpperCase()
    ...
    toString(); // Compliant
    foo()[0].toString(); // Noncompliant [[sc=5;ec=10]] {{There's no need to call "toString()" on an
    // array of String.}}
    bar()[0][0]
