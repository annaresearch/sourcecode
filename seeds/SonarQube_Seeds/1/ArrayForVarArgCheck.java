...
class ArrayForVarArgCheck {
  ...
  public void callTheThing(String s) throws IOException {
    doTheThing(
        new String[] {
          "s1", "s2"
        }); // Noncompliant {{Remove this array creation and simply pass the elements.}}
    // [[sc=16;ec=42]]
    ...
    doTheThing("s1");
    doTheThing2(
        new ArrayForVarArgCheckFoo[] {
          foo, foo
        }); // Noncompliant {{Disambiguate this call by either casting as "ArrayForVarArgCheckBar"
    // or "ArrayForVarArgCheckBar[]".}}
    doTheThing2(
        new ArrayForVarArgCheckFoo
            [12]); // Noncompliant {{Disambiguate this call by either casting as
    // "ArrayForVarArgCheckBar" or "ArrayForVarArgCheckBar[]".}}
    doTheThing2(
        new ArrayForVarArgCheckFoo
            [0]); // Noncompliant {{Disambiguate this call by either casting as
    // "ArrayForVarArgCheckBar" or "ArrayForVarArgCheckBar[]".}}
    ...
    java.nio.file.Files.write(
        java.nio.file.Paths.get("myPath"), new byte[0]); // Compliant, byte array is not a varargs
    java.nio.file.Files.write(
        java.nio.file.Paths.get("myPath"),
        new byte[] {' ', 'A', 'B', 'C'}); // Compliant, byte array is not a varargs
  }
