...
public class SwitchRedundantKeywordCheck {
  ...
  int switchExpression(String mode) {
    int nonCompliant =
        switch (mode) {
          case "a" -> {
            yield 1; // Noncompliant [[sc=9;ec=14;secondary=-1,+1]] {{Remove this redundant block
                     // and "yield".}}
          }
          default -> {
  ...
  int switchStatement(String mode) {
    ...
    switch (mode) {
      case "a" -> {
        result = 1;
        break; // Noncompliant [[sc=9;ec=15;secondary=-2,+1]] {{Remove this redundant block and
               // "break".}}
      }
      case "b" -> {
        result = 2;
    ...
    switch (mode) {
      ...
      case "c" -> { // Compliant, probably not the best code, but no choice to add an empty block if
                    // you want a case that does nothing.
      }
      default -> { // Compliant
        doSomethingElse();
        result = 2;
