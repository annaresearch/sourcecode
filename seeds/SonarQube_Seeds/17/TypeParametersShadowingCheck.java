...
public class TypeParametersShadowingCheck<T0> {
  class TypeParameterHidesAnotherType<T> {
    ...
    private <T>
        T method() { // Noncompliant [[sc=14;ec=15;secondary=-7]] {{Rename "T" which hides a type
  ...
  class MultipleHiding<T, Q> {
    public class Inner<
      ...
      // from the outer scope.}}
      private <T>
          T method2() { // Noncompliant [[secondary=-2]] {{Rename "T" which hides a type parameter
        // from the outer scope.}}
        return null;
      }

      private <Q>
          T method() { // Noncompliant [[sc=16;ec=17;secondary=-5]] {{Rename "Q" which hides a
  ...
  class TypeExtend<E extends Comparable<E>, Q> {
    public class Inner<
        E extends Q> { // Noncompliant [[sc=24;ec=25;secondary=-1]] {{Rename "E" which hides a type
      // parameter from the outer scope.}}
    }
