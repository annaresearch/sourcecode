...
public class DefaultEncodingUsageCheck {
  void myMethod(
    ...
    new String(
        bytes, 0, 1); // Noncompliant {{Remove this use of constructor "String(byte[],int,int)".}}
    ...
    new FileWriter(
        ...
        true); // java 11 - not resolved if not build with java 11, valid
    ...
    FileReader reader2 =
        new FileReader(file); // Noncompliant [[sc=30;ec=40]] {{Remove this use of constructor
    // "FileReader(File)".}}
    FileReader reader3 = new FileReader(file, StandardCharsets.UTF_8); // Compliant
    FileReader reader4 =
        new FileReader(fileDescriptor); // Noncompliant {{Remove this use of constructor
    // "FileReader(FileDescriptor)".}}
    FileReader reader5 =
        new FileReader(
            "filename"); // Noncompliant {{Remove this use of constructor "FileReader(String)".}}
    ...
    FileWriter writer3 =
        new FileWriter(file); // Noncompliant [[sc=30;ec=40]] {{Remove this use of constructor
    // "FileWriter(File)".}}
    FileWriter writer4 =
        new FileWriter(fileDescriptor); // Noncompliant {{Remove this use of constructor
    // "FileWriter(FileDescriptor)".}}
    FileWriter writer5 =
        new FileWriter(
            file,
            true); // Noncompliant {{Remove this use of constructor "FileWriter(File,boolean)".}}
    ...
    FileWriter writer8 =
        new FileWriter(
            "filename"); // Noncompliant {{Remove this use of constructor "FileWriter(String)".}}
    FileWriter writer9 =
        new FileWriter(
            "filename",
            true); // Noncompliant {{Remove this use of constructor "FileWriter(String,boolean)".}}
  ...
  void commons_fileutils(File file, CharSequence charSequence) throws IOException {
    FileUtils.readFileToString(
        file); // Noncompliant [[sc=15;ec=31]] {{Remove this use of "readFileToString".}}
  ...
  void commons_io_with_null(
    ...
    IOUtils.copy(
        input,
        writer,
        ((String) (((null))))); // Noncompliant [[sc=33;ec=54]] {{Replace this "null" with actual
    // charset.}}
  ...
  void commons_fileutils_with_null(File file, CharSequence charSequence) throws IOException {
    FileUtils.readFileToString(
        ...
        (java.nio.charset.Charset)
            null); // Noncompliant [[sc=38;ec=69]] {{Replace this "null" with actual charset.}}
