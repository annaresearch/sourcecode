...
class DateFormatWeekYearCheck {
  ...
  void quickFixes() throws ParseException {
    ...
    String result =
        new SimpleDateFormat("YYYY/MM/dd")
            .format(date); // Noncompliant [[sc=42;ec=54;quickfixes=qf1]]
    // fix@qf1 {{Replace year format}}
    // edit@qf1 [[sc=43;ec=47]] {{yyyy}}
    result =
        new SimpleDateFormat("YYYY").format(date); // Noncompliant [[sc=35;ec=41;quickfixes=qf2]]
    // fix@qf2 {{Replace year format}}
    // edit@qf2 [[sc=36;ec=40]] {{yyyy}}
    result =
        new SimpleDateFormat("  Y/MM/dd")
            .format(date); // Noncompliant [[sc=35;ec=46;quickfixes=qf3]]
  ...
  void useSimpleDateFormat() throws ParseException {
    ...
    String result =
        new SimpleDateFormat("YYYY/MM/dd")
            .format(
                date); // Noncompliant {{Make sure that week Year "YYYY" is expected here instead of
    // Year "yyyy".}}
    result =
    ...
    result =
        new SimpleDateFormat("  Y/MM/dd")
            .format(
                date); // Noncompliant {{Make sure that week Year "Y" is expected here instead of
    // Year "y".}}
    result = new SimpleDateFormat("yyyy/MM/dd").format(date); // Yields '2015/12/31' as expected
    result =
        new SimpleDateFormat("YYYY-ww")
            .format(date); // compliant, 'Week year' is used along with 'Week of year'. result =
    // '2016-01'
    result =
  ...
  public void useDateTimeFormatter() {
    ...
    DateTimeFormatter.ofPattern(
        nonCompliantAndNonFinalPattern); // Compliant FN Patterns in non-final String are not
    // processed
    ...
    DateTimeFormatter.ofPattern(
        " YY/MM/dd ".trim()); // Compliant FN patterns from method invocations are not processed
    ...
    DateTimeFormatter.ofPattern(
        " YY/MM/dd ".trim(),
        Locale.ENGLISH); // Compliant FN patterns from method invocations are not processed

    DateTimeFormatter.ofPattern(
        "YYYY"); // Noncompliant [[sc=33;ec=39]] {{Make sure that week Year "YYYY" is expected here
    // instead of Year "yyyy".}}
    DateTimeFormatter.ofPattern(
        "  Y/MM/dd"); // Noncompliant [[sc=33;ec=44]] {{Make sure that week Year "Y" is expected
    // here instead of Year "y".}}
    DateTimeFormatter.ofPattern(
        "YY"); // Noncompliant [[sc=33;ec=37]] {{Make sure that week Year "YY" is expected here
    // instead of Year "yy".}}
    DateTimeFormatter.ofPattern(
        "Y"); // Noncompliant [[sc=33;ec=36]] {{Make sure that week Year "Y" is expected here
    // instead of Year "y".}}
    DateTimeFormatter.ofPattern(
        NON_COMPLIANT_PATTERN); // Noncompliant [[sc=33;ec=54]] {{Make sure that week Year "YYYY" is
    // expected here instead of Year "yyyy".}}
    DateTimeFormatter.ofPattern(
        "YYYY",
        Locale
            .ENGLISH); // Noncompliant [[sc=33;ec=39]] {{Make sure that week Year "YYYY" is expected
    // here instead of Year "yyyy".}}
    DateTimeFormatter.ofPattern(
        "  Y/MM/dd",
        Locale.ENGLISH); // Noncompliant [[sc=33;ec=44]] {{Make sure that week Year "Y" is expected
    // here instead of Year "y".}}
    DateTimeFormatter.ofPattern(
        "YY",
        Locale.ENGLISH); // Noncompliant [[sc=33;ec=37]] {{Make sure that week Year "YY" is expected
    // here instead of Year "yy".}}
    DateTimeFormatter.ofPattern(
        "Y",
        Locale.ENGLISH); // Noncompliant [[sc=33;ec=36]] {{Make sure that week Year "Y" is expected
    // here instead of Year "y".}}
    DateTimeFormatter.ofPattern(
        NON_COMPLIANT_PATTERN,
        Locale
            .ENGLISH); // Noncompliant [[sc=33;ec=54]] {{Make sure that week Year "YYYY" is expected
    // here instead of Year "yyyy".}}
  }
  ...
  class NonCompliantChildOfSimpleDateFormat extends SimpleDateFormat {
    public NonCompliantChildOfSimpleDateFormat() {
      super("YYYY"); // Noncompliant {{Make sure that week Year "YYYY" is expected here instead of
      // Year "yyyy".}}
    }
  }
}
