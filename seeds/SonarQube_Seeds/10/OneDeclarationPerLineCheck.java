...
class OneDeclarationPerLineCheck {
  ...
  private static final int STATIC_FINAL_A = 1,
      STATIC_FINAL_B = 1; // Noncompliant {{Declare "STATIC_FINAL_B" on a separate line.}}

  static {
    int staticA, staticB = 1; // Noncompliant {{Declare "staticB" on a separate line.}}
    int staticC = 1;
    int staticD; // Noncompliant {{Declare "staticD" on a separate line.}}
  }

  int a;
  int b;
  int c;
  int d; // Noncompliant [[sc=14;ec=15;secondary=+0,+0]] {{Declare "b" and all following
  // declarations on a separate line.}}
  int e;

  private int j1 = -1,
      j2 // Noncompliant [[sc=24;ec=26;secondary=+2,+4]] {{Declare "j2" and all following
      // declarations on a separate line.}}
      ,
      j3 = 1,
      j4;
  private int i1 = -1,
      i2,
      i3 = 1; // Noncompliant [[sc=24;ec=26;secondary=+0]] {{Declare "i2" and all following
  // declarations on a separate line.}}
  ;
  ...
  public void method() {
    ...
    for (int i = 0, j = 0; i < 1; i++) { // Compliant in statment declaration
      ...
      int forAfterC;
      int forAfterD = forB; // Noncompliant {{Declare "forAfterD" on a separate line.}}
    }
  }
  ...
  void mixedAreReportedTogether() {
    int k;
    int l;
    int m, n; // Noncompliant [[sc=16;ec=17;secondary=+0,+0;quickfixes=qf_mixed1]]
    ...
    int k1;
    int l1, m1;
    int n1; // Noncompliant [[sc=17;ec=19;secondary=+0,+0;quickfixes=qf_mixed2]]
  ...
  class TestQuickFix {
    int i;
    int j; // Noncompliant [[sc=16;ec=17;quickfixes=qf1]]
    ...
    int j2;
    int k2;
    int l2; // Noncompliant [[sc=17;ec=19;quickfixes=qf2]]
    ...
    void m() {
      int i7;
      m();
      int j7; // Noncompliant [[sc=24;ec=26;quickfixes=qf17]]
  ...
  int correct;
  int indentation; // Noncompliant [[sc=20;ec=31;quickfixes=qf_indentation]]
  // fix@qf_indentation {{Declare on separated lines}}
  // edit@qf_indentation [[sc=15;ec=16]] {{\n  }}

  int no;
  int spaceBefore; // Noncompliant [[sc=13;ec=24;quickfixes=qf_indentation2]]
  ...
  int i7;
  int i8;
  int i9, i10; // Noncompliant {{Declare "i10" on a separate line.}}
  // For corner case test, last variable must be a "dubble" declaration with no simple declaration
  // after ; and end token on next line.
