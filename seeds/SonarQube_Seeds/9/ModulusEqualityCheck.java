...
class ModulusEqualityCheck {
  ...
  void myMethod(int x, int y, Integer z) {
    b =
        x % 2
            == 1; // Noncompliant [[sc=12;ec=13]] {{The results of this modulus operation may not be
    // positive.}}
    ...
    b =
        z.intValue() % 2
            == 1; // Noncompliant {{The results of this modulus operation may not be positive.}}
    b = x % 2 == y;
    b = x % 2 == 0;
    b = x % 2 != 1;
    int i = 5;
  ...
  void myOtherMethod(Collection c, List l, String s, int[] a, Queue<String> q) {
    ...
    b =
        s.indexOf("") % 2
            == -1; // Noncompliant {{The results of this modulus operation may not be negative.}}
  }
}
