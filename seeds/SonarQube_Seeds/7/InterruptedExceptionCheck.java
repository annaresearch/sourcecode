  ...
  public void runInterrupted() {
    ...
    try {
      while (true) {
        ...
        InterruptedException e) { // Noncompliant {{Either re-interrupt this method or rethrow the
      // "InterruptedException" that can be caught here.}}
      LOGGER.log(Level.WARN, "Interrupted!", e);
      // clean up state...
...
class Interruptable {
  ...
  public void catchGenericException() throws InterruptedException {
    ...
    try {
      throwsInterruptedException();
    } catch (Throwable e) { // Noncompliant {{Either re-interrupt this method or rethrow the
      // "InterruptedException" that can be caught here.}}
      LOGGER.log(Level.WARN, "Interrupted!", e);
    }
