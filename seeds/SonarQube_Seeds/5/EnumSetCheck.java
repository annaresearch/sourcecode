...
class EnumSetCheck {
  ...
  public void doSomething(Set<COLOR> param) { // compliant, we ignore parameters.
    Set<COLOR> warm =
        new HashSet<COLOR>(); // Noncompliant [[sc=23;ec=43]] {{Convert this Set to an EnumSet.}}
    warm.add(COLOR.RED);
    ...
    Set warm2 =
        new HashSet<COLOR>(); // Noncompliant [[sc=17;ec=37]] {{Convert this Set to an EnumSet.}}
    ...
    Set<COLOR> ports =
        new HashSet<>(); // Noncompliant [[sc=24;ec=39]] {{Convert this Set to an EnumSet.}}
    Set<COLOR> ports4 =
        Sets.immutableEnumSet(
            COLOR.RED); // Compliant - guava use an enum set with constraint of immutability
    Set<COLOR> ports5 =
        Sets.immutableEnumSet(
            Lists.newArrayList(
                COLOR.RED)); // Compliant - guava use an enum set with constraint of immutability
    Set<COLOR> ports6 =
        Sets.newHashSet(COLOR.RED); // Noncompliant {{Convert this Set to an EnumSet.}}
    ...
    Set<COLOR> col7 =
        ImmutableSet.of(
            ...
            COLOR.GREEN,
            COLOR.ORANGE,
            COLOR.BLUE); // Noncompliant

    Set<COLOR> col8 = Set.of(COLOR.RED); // Noncompliant
    Set set =
        EnumSet.of(
            E.E1, E.E2, E.E3, E.E4, E.E5,
            E.E6); // Compliant, overload of(E first, E... rest) properly resolved

    // We are not computing the exact runtime type when the initializer is not a method
    ...
    Set<COLOR> col3_3 =
        Collections.unmodifiableSet(
            ports); // FN, we are not computing the runtime type of the argument

    Set<COLOR> ternaryInit =
        param.isEmpty() ? EnumSet.allOf(COLOR.class) : EnumSet.of(COLOR.GREEN); // Compliant
    Set<COLOR> ternaryInit2 =
        param.isEmpty() ? new HashSet<>() : new HashSet<>(); // FN: acceptable corner case
